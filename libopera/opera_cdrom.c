    ...
    case CDROM_CMD_READ_TOC:
      if((cd_->xbus_status & CDST_TRAY) &&
         ...
         (cd_->xbus_status & CDST_SPIN))
        {
          ...
          //printf("READ_TOC: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X \n", cd_->status[0], cd_->status[1], cd_->status[2], cd_->status[3], cd_->status[4], cd_->status[5], cd_->status[6], cd_->status[7], cd_->status[8], cd_->status[9]);
        }
      else
        {
          cd_->status_len   = 2;
